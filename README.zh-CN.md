...
- 面向实际使用的桥接能力：支持 `/help`、`/status`、`/reset`、处理中反馈、本地日志和可选 verbose 事件日志。
...
- 终端日志保留时间戳格式
- 同样内容会写入 `logs/bridge-YYYY-MM-DD.log`
- 当 `CODEX_VERBOSE_EVENTS=true` 时，还会写入 `logs/requests/<requestId>.codex-events.jsonl`
